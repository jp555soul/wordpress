...
/* Adds multi-thumbnail support for posts */
if (!class_exists('MultiImage')) {
	...
	class MultiImage {
		...
		public function add_attachment_field($form_fields, $post){
			...
			$link = sprintf('<a id="%4$s-%1$s-thumbnail-%2$s" class="%1$s-thumbnail link-multi-set" href="?thumb_id=%2$s&id=%1$s&post_type=%4$s&nonce=%5$s">Set as %3$s</a>', $this->id, $post->ID, $this->label, $this->post_type, $ajax_nonce);
		...
		public static function get_the_post_thumbnail_src($post_type, $thumb_id, $post_id) {
			global $id;
			$post_id = (NULL === $post_id) ? $id : $post_id;
			$post_thumbnail_id = self::get_post_thumbnail_id($post_type, $thumb_id, $post_id);
			if ($post_thumbnail_id) {
		...
		private function post_thumbnail_html($thumbnail_id = NULL) {
			...
			if ($thumbnail_id && get_post($thumbnail_id)) {
				...
				if (!empty($thumbnail_html)) {
					...
					$content .= sprintf('<p class="hide-if-no-js"><a href="#" id="remove-%1$s-%2$s-thumbnail" onclick="MultiImageRemoveThumbnail(\'%2$s\', \'%1$s\', \'%4$s\');return false;">%3$s</a></p>', $this->post_type, $this->id, esc_html__( "Remove {$this->label}" ), $ajax_nonce);
				}
