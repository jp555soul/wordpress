		...
		$attachments = get_posts($args);
			...
			if ($attachments) {
				...
				foreach ($attachments as $attachment) {
					...
					$out .= $i == 0 ? '<a href="'.$href.'" data-rel="prettyPhoto[portfolio]" title="'.get_the_title($id).'"><div class="caption">'.get_the_title($id).'</div>'.get_the_post_thumbnail( $id, 'portfolio_thumb' ).'</a>' : '<a href="'.$href.'" data-rel="prettyPhoto[portfolio]" title="'.get_the_title($id).'"></a>' ;
